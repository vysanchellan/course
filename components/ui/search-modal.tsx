...
export function SearchModal({ open, onClose }: SearchModalProps) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="fixed inset-0 bg-dusk/60 backdrop-blur-sm" />
      <div
        className="relative w-full max-w-lg bg-parchment border border-ink/15 rounded-md shadow-2xl overflow-hidden"
        ...
        <div className="border-b border-ink/10">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <input
              ...
              className="flex-1 bg-transparent font-mono text-sm text-ink placeholder:text-muteddark/50 focus:outline-none"
            />
            <kbd className="hidden md:inline-flex font-mono text-[11px] text-muteddark bg-parchment2 px-2 py-0.5 rounded">
              ESC
            </kbd>
          </div>
        </div>
            ...
            results.map((lesson, i) => {
              ...
              return (
                <Link
                  ...
                  onClick={onClose}
                  className={cn(
                    "flex items-center gap-3 px-4 py-3 border-b border-ink/5 last:border-0 transition-colors",
                    i === selectedIndex ? "bg-parchment2" : "hover:bg-parchment2/50"
                  )}
                >
